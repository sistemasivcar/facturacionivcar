...
async function incStock(artId, cant) {
  ...
  if (article) {
    ...
    console.error("no existe articulo");
  }
}
...
export default {
  add: async (req, res, next) => {
    ...
    try {
      ...
      for (let i = 0; i < venta.detalles.length; i++) {
        articulo = await checkStock(
          venta.detalles[i].cantidad,
          venta.detalles[i]._id
        );
        if (articulo) {
          return res
            .status(400)
            .json({
              message: "No hay stock sufuciente para el producto: " + articulo,
            });
        }
      }
  ...
  update: async (req, res, next) => {
    try {
      const venta = await Venta.findByIdAndUpdate(req.query._id, req.body, {
        new: true,
      });
